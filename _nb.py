...
def build_statistics_rules():
    ...
    dream_special = '/'.join(sorted(DREAM_FIRST_SPECIAL_IDENTITIES))
    rate_rules = {k: f"{v[0]} ÷ {v[1]}" for k, v in RATE_COLS.items()}
    rate_rules['说明'] = '分母为 0 时比率记为 0；输出格式为百分比保留一位小数'
    return {
        '说明': (
            '本文档描述 statistics.ipynb 中所有统计判定与汇总规则，与代码逻辑一致。'
            '对局以「日期 + 版型」唯一标识；好人侧指阵营非「狼人」「狼人混」的玩家。'
        ),
        '数据来源与处理流程': {
            '输入': f'Excel 表单（{dataset_desc}）',
            '步骤': [
                '读取 Excel 后，用身份推导阵营并覆盖原阵营列',
                '导出原始数据 JSON（去掉「技能使用情况」「其他」列，阵营列紧跟身份列）',
                '为放逐票列补充目标阵营',
                '生成各类标记列并汇总玩家统计、版型统计、同边统计',
                '输出 JSON / Excel 至 data 目录',
                '导出本规则文档至 规则.json',
            ],
        },
        '阵营推导': {
            ...
            '映射': [
                ...
                '狼人混 → 狼人混',
                f'{wolf_id_text}，或身份含「盗宝」「机械」「傀儡」 → 狼人',
                '其余身份 → 神职',
            ],
            '好人侧判定': '阵营不为「狼人」「狼人混」',
            '狼人侧判定': '阵营为「狼人」或「狼人混」',
        ...
        '存活天数': {
            '个人存活天数': (
                '从 D1状态 到 D5状态 从左扫描，首个有非空状态记录的天数即为存活天数；'
                '若全无，则等于该对局持续天数'
            ),
            '对局持续天数': (
                '同日期+版型对局中，任一玩家在某天有状态记录，则该天计入；取最后有记录的天数'
            ),
            '平均存活天数': '存活天数总和 ÷ 出场次数',
            '狼人平均存活天数': '狼侧存活天数总和 ÷ 狼次数',
            '好人平均存活天数': '好人侧存活天数总和 ÷ 好人次数',
        },
        '基础对局标记': {
            '胜利标记': '对局「胜利」为「狼」或「好人」',
            '狼标记': '阵营为狼人侧',
            ...
            '神职身份标记': '阵营为神职',
            '机械狼标记': (
                f'身份为机械狼或机械子身份（{"、".join(JIXIE_SUB_ROLES)}）'
            ),
            '机械子身份标记': '身份分别等于对应机械子身份时记 1',
            '好人胜利标记': '阵营为平民或神职，且对局胜利方为「好人」',
            '狼胜利标记': '阵营为狼人侧，且对局胜利方为「狼」',
        },
        '女巫与毒药': {
            '女巫毒师标记': f'身份为{"或".join(POISON_ROLES)}',
            '压毒标记': (
                f'身份为{"/".join(POISON_ROLES)}，且所有含「夜目标1」的列均为空或「救」'
            ),
            '毒药使用标记': (
                f'身份为{"/".join(POISON_ROLES)}，且任一带「夜目标1」的列有非空、非「救」的有效值'
            ),
            '毒狼人标记': '有毒药使用标记，且上述任一有效夜目标1 对应玩家阵营为「狼人」',
            '盲毒标记': (
                f'身份为{"/".join(POISON_ROLES)}，且 D1夜目标1 非空且不为「救」'
            ),
            '盲毒狼人标记': '有盲毒标记，且 D1夜目标1 对应玩家阵营为「狼人」',
            '被盲毒标记': 'D1状态 为「被毒」',
            '非狼被盲毒标记': '有被盲毒标记，且阵营不为「狼人」',
        },
        '猎人与守卫与摄梦人': {
            '猎人标记': '身份为猎人',
            '猎人次数': '身份为猎人的总局数（猎人标记之和）',
            '枪标记': '有猎人标记，且攻击技能为「√」',
            '守卫标记': '身份为守卫',
            '守卫平安夜标记': '有守卫标记，且防御技能为「√」',
            '摄梦人标记': '身份为摄梦人',
            '摄狼人标记': '有摄梦人标记，且攻击技能为「√」',
            '首摄狼人标记': (
                f'摄梦人 D1夜目标1 非空，且目标身份不属于好人身份集合（{good_id_text}）'
            ),
            '首摄狼王盗宝猎人标记': (
                f'摄梦人 D1夜目标1 非空，且目标身份为{dream_special}'
            ),
            '猎人技能次数': (
                f'身份为{HUNTER_ROLE}，任一带「技能」的列有非空有效目标（非「√」「×」等标记）'
            ),
            '猎人技能狼人次数': (
                f'身份为{HUNTER_ROLE}，上述技能目标在同对局对应玩家阵营为「狼人」的次数'
            ),
            '猎人技能正确率': '猎人技能狼人次数 ÷ 猎人技能次数',
        },
        '预言家与警徽票': {
            '起跳预言家标记': [
                '身份为预言家或通灵师，且 D1验人 有有效值；或',
                (
                    '好人侧、非预言家/通灵师，D1验人 有有效值，'
                    '且同对局所有预言家/通灵师的 D1验人 均为空（视为悍跳/起跳）'
                ),
            ],
            '警下标记': '好人侧，且 D1发言顺序 为「警下」',
            '第一轮警徽票正确标记': (
                '好人侧、D1警下投票 有有效值、D1发言顺序 为「警下」，'
                '且投票目标为同对局起跳预言家'
            ),
            '第二轮警徽票正确标记': (
                '好人侧、D1二轮票 有有效值，且投票目标为同对局起跳预言家'
            ),
            '警徽票正确标记': '第一轮与第二轮警徽票正确标记之和',
            '警徽票次数': (
                '好人侧 D1警下投票 有有效值'
                '+ 好人侧 D1二轮票 有有效值（若存在二轮票列）'
            ),
            '平民警徽票次数': '「警徽票次数」规则，且该局身份为「平民」',
            '平民警徽票正确次数': '「警徽票正确标记」规则，且该局身份为「平民」',
            '警徽票错误记录': (
                '有警徽票标记且无警徽票正确标记时，按「预言家/通灵师姓名-悍跳狼姓名」'
                '记一条，多条用逗号连接'
            ),
            '盗宝大师通灵师局排除': (
                f'版型为{DAOBAO_MASTER_BANXING}且局内有{DAOBAO_MEDIUM_ROLE}时，'
                '不统计警徽票次数、警徽票正确次数及由此派生的平民警徽票统计'
            ),
        },
        '悍跳与自爆': {
            '悍跳标记': '狼人侧，且 D1验人 非空',
            '自爆标记': '狼人侧，且 D1–D5 状态 中任一天为「自爆」',
        },
        '放逐票': {
            '可投票身份': '非预言家、非通灵师',
            '放逐票次数（单次）': '好人侧、对应 D{n}放逐票 有有效值、且身份可投票',
            '正确放逐票（常规）': '满足放逐票次数条件，且投票目标阵营为「狼人」',
            '正确放逐票（诡术之境特殊）': {
                '版型': ARCANE_BANXING,
                '触发': (
                    f'身份为{ARCANE_MASTER_ROLE}的玩家，在同一 D{{n}} 下 '
                    'D{n}夜目标1 与 D{n}夜目标2 均有值 A、B，且 A、B 对应玩家阵营一狼一好人'
                ),
                '正确条件': (
                    '同对局、同 D{n} 下，好人侧可投票玩家在 D{n}放逐票1 或 D{n}放逐票2 '
                    '投给 A、B 中的好人，亦记为正确放逐票（与常规投狼规则叠加，满足任一即记 1）'
                ),
            },
            '特殊放逐票': f'「{SPECIAL_ORDER_EXILE_COL}」列有有效值，且满足可投票条件',
            '特殊正确放逐票': '特殊放逐票有效，且目标阵营为「狼人」',
            '正确放逐票总计': (
                'D1–D5 各天放逐票1/2 的正确标记 + 特殊正确放逐票标记 之和'
            ),
            '放逐票次数总计': (
                'D1–D5 各天放逐票1/2 的放逐票标记 + 特殊放逐票标记 之和'
            ),
            '无预通生推可统计日': {
                '预通存活': '同对局身份为预言家或通灵师的玩家，该天 D{n}状态 为空',
                '预通被放逐首日': (
                    '上述玩家该天 D{n}状态 首次出现「被放逐」'
                ),
                '可统计': '该天不满足上述任一条件；对局无预言家/通灵师时各天均可统计',
            },
            '无预通生推正确放逐票': (
                '在常规/诡术正确放逐票规则下，仅统计无预通生推可统计日的 D{n}放逐票1/2 正确标记之和'
            ),
            '无预通放逐票': (
                '在放逐票次数规则下，仅统计无预通生推可统计日的 D{n}放逐票1/2 放逐票标记之和'
            ),
            '平民放逐票次数': '「放逐票次数总计」规则，且该局身份为「平民」',
            '平民正确放逐票次数': '「正确放逐票总计」规则，且该局身份为「平民」',
        },
         '放逐票目标阵营补充': {
            '范围': 'D1–D5 的「放逐票1」「放逐票2」，以及「特殊放逐票」',
            '方法': '按同日期、投票目标姓名匹配该玩家阵营',
            '特殊': '投票目标为「预通」时不计入阵营（记为空）；无法匹配时记为「错误」',
        },
        '首验与混血儿': {
            '被首验标记': (
                f'同日期+版型中，{"、".join(FIRST_CHECK_ROLES)}的 D1夜目标1 指向该玩家'
            ),
            '混血儿榜样标记': '同日期+版型中，好人混/狼人混的 D1夜目标1 指向该玩家',
        },
        '假金（悍跳发金水）': {
            '发假金事件': '狼人侧 D1验人 非空，验人结果非「查杀」，被验玩家阵营非狼人/狼人混',
            '假金标记（接狼人金水）': '非狼人、非狼人混阵营被悍跳狼发假金的玩家',
            '接狼人金水文本': '按被发假金玩家汇总发假金悍跳狼姓名及次数',
        },
        '拉杆（通灵师板）': {
            '拉杆标记': '同对局有身份为「通灵师」的玩家，且该玩家有悍跳标记，D1验人结果非「金水」非「查杀」',
            '拉杆成功标记': '有拉杆标记，且 D1验人 对应玩家在同对局的身份等于 D1验人结果',
            '拉杆记录': '拉杆成功时按「日期-姓名-D1验人-D1验人结果」汇总，多条用逗号连接',
        },
        '红狼与狼刀': {
            '技能狼标记': '阵营为狼人（不含狼人混），且身份不为「狼」',
            '非盗宝红狼标记': (
                f'身份为{"或".join(RED_WOLF_IDENTITIES)}，且版型不是「{DAOBAO_MASTER_BANXING}」'
            ),
            '自刀标记': '非盗宝红狼，D1狼刀 有有效值且目标为自己姓名',
            '狼队友自刀标记': '非盗宝红狼，D1狼刀 有有效值且目标为同对局另一名非盗宝红狼',
            '首刀女巫标记': '狼人阵营，D1狼刀 有有效值且目标为同对局身份为「女巫」的玩家',
        },
        '文本汇总字段': {
            '参与场次': '按姓名汇总所有对局，格式「日期（版型，身份）」、分隔，按日期降序',
            '胜利场次': '同上，仅统计胜利标记为 1 的对局',
            '首验': '预言家/通灵师 D1夜目标1 按目标汇总为「姓名（次数）」',
            '被首验': '被预言家/通灵师首验的玩家，汇总首验者「姓名（次数）」',
            '盲毒狼人': '女巫/毒师盲毒到狼人的目标汇总',
            '毒药记录': (
                '有毒药使用标记时，按所有含「夜目标1」的非空、非「救」目标'
                '对应玩家身份汇总，格式「身份（次数）」、分隔，按次数降序'
            ),
            '猎人技能记录': (
                f'身份为{HUNTER_ROLE}，含「技能」列有有效目标时，'
                '按目标玩家身份汇总，格式「身份（次数）」、分隔，按次数降序'
            ),
            '混血儿记录': (
                '身份为好人混/狼人混时，按 D1夜目标1 对应玩家汇总，'
                '目标阵营为狼人侧记「狼」、其余记「好人」，'
                '格式「姓名（狼/好人，次数）」、分隔，按次数降序'
            ),
            '接狼人金水': '被发假金玩家汇总发假金悍跳狼',
            '拉杆记录': '拉杆成功明细，格式「日期-姓名-D1验人-D1验人结果」，逗号连接',
            '警徽票错误记录': '警徽票投错明细，格式「预言家/通灵师姓名-悍跳狼姓名」，逗号连接',
        },
        '玩家统计比率': rate_rules,
        '版型统计': {
            '对局拆分': {
                '预女猎白混': (
                    '若局内有狼人混 → 记「预女猎白狼人混」；'
                    '若有好人混 → 记「预女猎白好人混」；否则不单独拆分'
                ),
                '盗宝大师': (
                    f'按局内出现的盗宝子身份（{"、".join(DAOBAO_ROLES)}）分别记场次'
                ),
                '机械狼': '记「机械狼」总版型，并按局内出现的机械子身份分别记场次',
                '其他版型': '原版型名直接统计',
            },
            '单版型指标': {
                '总场次': '该版型出现次数',
            ...
            '汇总版型': [
                '「预女猎白混」= 预女猎白好人混 + 预女猎白狼人混 的场次与胜率加权汇总',
                '「盗宝大师」= 各盗宝子身份 的场次与胜率加权汇总',
            ],
            '缺失版型': '未出现过的盗宝子身份、机械子身份及预女猎白相关版型补 0 场次',
        },
        '多赛季版型合并': {
            '总场次与狼赢次数': '各赛季同版型直接求和',
            '平均持续天数': '按各赛季「总场次 × 平均持续天数」加权平均',
            '狼人胜率': '合并后狼赢次数 ÷ 合并后总场次',
        },
        '同边统计': {
            '选手范围': '出场次数 ≥ 6 的选手',
            '配对范围': '同日期+版型对局内所有选手两两组合',
            '同场次数': '两选手同局出现的次数',
            '同边判定': '两选手阵营同属狼人侧或同属好人侧',
            '同边次数': '同场且同边的次数',
            '同边狼人次数': '同场且均为狼人侧的次数',
            '同狼人次数': '同场且均为狼人侧（用于好人胜率分母）',
            '同好人次数': '同场且均为好人侧',
            '同边率': '同边次数 ÷ 同场次数',
            '同边狼人胜率': '同边均为狼人且狼赢的次数 ÷ 同边狼人次数',
            '同边好人胜率': '同边均为好人且好人赢的次数 ÷ 同好人次数',
        },
        '原始数据导出': {
            '删除列': '技能使用情况、其他',
            '列顺序': '阵营列紧跟身份列之后',
            '行顺序': '保持 Excel 原始顺序（按座位/录入顺序）',
        },
        '角色常量': {
...
def _get_d1_check_result_col(data):
    if 'D1验人结果' in data.columns:
        return 'D1验人结果'
    if 'D1结果' in data.columns:
        return 'D1结果'
    return None
...
def summarize_sheriff_vote_error_records(data):
    """警徽票错误记录：有警徽票标记且无警徽票正确标记时，汇总「预言家/通灵师-悍跳狼」。"""
    empty = pd.DataFrame({'姓名': pd.Series(dtype=str), '警徽票错误记录': pd.Series(dtype=str)})
    required = {
    ...
    return (
        ...
        .groupby('姓名', as_index=False)['_record']
        .agg(lambda records: ','.join(records))
        .rename(columns={'_record': '警徽票错误记录'})
    )
...
def add_markers(data):
    ...
    data['枪标记'] = (data['猎人标记'] & (data['攻击技能'] == '√')).astype(int)
    data = assign_hunter_skill_markers(data)
    data['守卫标记'] = (role == '守卫').astype(int)
    data['守卫平安夜标记'] = (data['守卫标记'] & (data['防御技能'] == '√')).astype(int)
    data['摄梦人标记'] = (role == '摄梦人').astype(int)
    data['摄狼人标记'] = (data['摄梦人标记'] & (data['攻击技能'] == '√')).astype(int)
...
def build_tongbian_stats(data, stat_df, min_appearances=6):
    ...
    for (_, _), group in data.groupby(['日期', '版型']):
        players = group[['姓名', '阵营']].drop_duplicates('姓名')
